...
@section('content')
    ...
    <div class="container mt-0">
        ...
        <h2 class="text-center">{{ __('Client Cars') }} </h2>
        @if(Auth::user()->role->name === 'Admin' )
            <a href="{{route('cars_clients.create' , app()->getLocale())}}"
        ...
        <div class="row  justify-content-center">
            ...
            <div class="col-md-6">
                <div class="form-group  mb-3">
                    ...
                    <form
                            id="search-form-text"
                            action="{{ route('cars_clients.index', ['language' => app()->getLocale()]) }}"
                            method="get">
            ...
            <div class="col-md-3">
                <form
                        id="search-form-number" action="{{ route('cars_clients.index', ['language' => app()->getLocale()]) }}"
                        method="get">
            ...
            <table id="table" class="table">
                ...
                @foreach ($records as $record)
                    ...
                    <tr>
                        ...
                        <td>{{ $record->car_quality_number }}</td>
                        <td>{{ $record->car_code }}</td>
                        <td>
                            <form
                                    action="{{ route('cars_clients.destroy', ['language' => app()->getLocale(), 'cars_client' => $record->id]) }}"
                                    method="post">
                                <input type="hidden" name="id" value="{{ $record->id }}">

                                <a href="{{ route('cars_clients.show', ['language' => app()->getLocale(), 'cars_client' => $record->id]) }}"
                                ...
                                @if(Auth::user()->role->name === 'Admin' )
                                    ...
                                    <a href="{{ route('cars_clients.edit', ['language' => app()->getLocale(), 'cars_client' => $record->id]) }}"
